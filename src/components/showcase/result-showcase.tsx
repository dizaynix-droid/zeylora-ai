      ...
      <div className="section-shell">
        <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr] lg:items-end">
          <div>
            ...
            <h2 className="mt-3 text-2xl font-black leading-tight tracking-tight text-white md:mt-4 md:text-5xl">
              Lead with sharper, brighter, more premium product visuals.
            </h2>
            <p className="mt-3 max-w-xl text-sm leading-6 text-slate-300 md:mt-4 md:text-base md:leading-8">
          ...
          <div className="glass-panel rounded-2xl p-3 md:rounded-[2rem] md:p-5">
        ...
        <div className="mt-6 grid gap-4 lg:mt-8 lg:grid-cols-2 lg:gap-5">
          {featuredExamples.map((example, index) => (
            <article
              key={example.slug}
              className="overflow-hidden rounded-2xl border border-white/10 bg-[linear-gradient(135deg,rgba(255,255,255,.07),rgba(32,211,255,.035),rgba(139,92,246,.045))] p-2 shadow-cinematic md:rounded-[2rem] lg:p-4"
            >
              <BeforeAfterSlider
              ...
              <div className="grid gap-3 p-2 pt-4 lg:grid-cols-[1fr_auto] lg:items-end lg:p-4">
                <div>
                  ...
                  <p className="mt-2 text-sm leading-6 text-slate-300 md:mt-4 md:leading-7">{example.promise}</p>
                ...
                <div className="min-w-0 lg:min-w-[190px]">
                  ...
                  <Button href="#upload" className="mt-3 h-11 w-full md:mt-4 md:h-12">
        ...
        <div className="mt-4 grid gap-4 md:mt-5 md:grid-cols-2 xl:grid-cols-4">
          {compactExamples.map((example) => (
            <article
              key={example.slug}
              className="overflow-hidden rounded-2xl border border-white/10 bg-white/[0.045] p-2 shadow-cinematic md:rounded-[1.75rem] md:p-3"
            >
              <BeforeAfterSlider
                before={example.before}
                after={example.after}
                ...
                priority={false}
              />
              <div className="p-2 pt-3 md:pt-4">
                <p className="text-xs font-black uppercase text-cyan">{example.tool}</p>
                <h3 className="mt-2 text-lg font-black leading-tight text-white md:text-xl">{example.afterLabel}</h3>
