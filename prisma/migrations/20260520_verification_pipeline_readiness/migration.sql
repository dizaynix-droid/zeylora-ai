-- Idempotent safety migration for the email verification pipeline.
-- This intentionally mirrors the runtime readiness guard so production can be repaired safely.
...
-- Existing production databases may already have these tables from earlier
-- iterations. CREATE TABLE IF NOT EXISTS will not add missing columns, so keep
-- this migration idempotent and explicitly repair the verification shape.
